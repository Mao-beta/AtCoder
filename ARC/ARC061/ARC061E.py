...
class Dijkstra():
    ...
    * ベルマンフォード法より高速なので、負のコストがないならばこちらを使うとよい
    ...
    def shortest_path(self, s):
        ...
        d = [float("inf")] * self.V
        d[s] = 0
        heapq.heappush(que, f(0, s))  # 始点の(最短距離, 頂点番号)をヒープに追加する

        while len(que) != 0:
            cost, v = divmod(heapq.heappop(que), INF)
            ...
            for i in range(len(self.G[v])):
                ...
                if d[e.to] > d[v] + e.cost:
                    ...
                    heapq.heappush(que, f(d[e.to], e.to))  # キューに新たな最短経路の候補(最短距離, 頂点番号)の情報をpush
        return d
...
def main():
    ...
    S = set(f(i, 0) for i in range(1, N+1))
    for p, q, c in PQC:
        S.add(f(p, c))
        S.add(f(q, c))
    ...
    G = Dijkstra(len(D))
    for p, q, c in PQC:
        G.add(D[f(p, c)], D[f(q, c)], 0)
        G.add(D[f(q, c)], D[f(p, c)], 0)
        G.add(D[f(p, 0)], D[f(p, c)], 1)
        G.add(D[f(q, 0)], D[f(q, c)], 1)
        G.add(D[f(p, c)], D[f(p, 0)], 0)
        G.add(D[f(q, c)], D[f(q, 0)], 0)
    P = G.shortest_path(D[f(1, 0)])
    ans = P[D[f(N, 0)]]
    if ans == float("inf"):
        print(-1)
    else:
