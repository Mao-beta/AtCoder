...
def reflect_point(line: Line, p: Point) -> Point:
    """
    直線 line に対する点 p の鏡映像を返す。

    点 p の線分 line.p から line.q への射影を Fraction で正確に求め、
    そこから反射後の座標を計算して返します。
    四捨五入は行いません。
    """
    A, B = line.p, line.q
    AP = p - A
    AB = B - A
    t = Fraction(AP.dot(AB), AB.norm_sq())
    proj = A + t * (B - A)
    reflected = proj * 2 - p
    return reflected
...
if __name__ == '__main__':
    ...
    print("s1 intersects s2:", s1.intersects(s2))

    # --- Line の交点計算 ---
    l1 = Line(Point(0, 0), Point(4, 4))
    l2 = Line(Point(0, 4), Point(4, 0))
    print("Line intersection:", l1.intersection(l2))

    # --- Polygon の操作 ---
    pts = [Point(0, 0), Point(2, 0), Point(1, 1), Point(0, 2), Point(2, 2)]
    poly = Polygon(pts)
    hull = Polygon.convex_hull(pts)
    print("Polygon convex hull:", hull)
    print("Polygon area:", poly.area())
    print("Point (1,1) in polygon:", poly.contains(Point(1, 1)))
    print("Point (3,3) in polygon:", poly.contains(Point(3, 3)))

    # --- 点の反射 ---
    line = Line(Point(0, 0), Point(4, 4))
    p_reflected = reflect_point(line, Point(0, 4))
    print("Reflection of (0,4) over line through (0,0)-(4,4):", p_reflected)
