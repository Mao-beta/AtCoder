...
def main():
    ...
    for u, v in UV:
        if u != 0:
            ...
            zeros.add(v)
    D1 = G.shortest_path(1)
    DN = G.shortest_path(N)
    min01 = -1
    min01d = INF
    min0N = -1
    min0Nd = INF
    for z in zeros:
        if D1[z] < min01d:
            min01d = min(min01d, D1[z])
            min01 = z
        if DN[z] < min0Nd:
            min0Nd = min(min0Nd, DN[z])
            min0N = z

    ANS = []
    for i in range(1, N+1):
        ans = D1[N]
        ans = min(ans, min01d + 1 + DN[i])
        ans = min(ans, min0Nd + 1 + D1[i])
        ans = min(ans, min01d + 2 + min0Nd)
