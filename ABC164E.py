...
def main():
    N, M, S = NMI()
    UVAB = [NLI() for _ in range(M)]
    UVAB = [[x-1, y-1, *w] for x, y, *w in UVAB]
    CD = [NLI() for _ in range(N)]

    sum_a = sum([a for u, v, a, b in UVAB])

    def ua2v(u, a):
        return (sum_a + 1) * u + a

    G = Dijkstra(N * (sum_a + 1))
    for u, v, a, b in UVAB:
        for sil in range(sum_a+1):
            if sil >= a:
                G.add(ua2v(u, sil), ua2v(v, sil-a), b)
                G.add(ua2v(v, sil), ua2v(u, sil-a), b)

    for u, (c, d) in enumerate(CD):
        for sil in range(sum_a+1):
            if sil + c <= sum_a:
                G.add(ua2v(u, sil), ua2v(u, sil+c), d)

    S = min(S, sum_a)
    res = G.shortest_path(ua2v(0, S))

    for t in range(1, N):
        ans = 10**15
        for sil in range(sum_a+1):
            ans = min(ans, res[ua2v(t, sil)])
        print(ans)
