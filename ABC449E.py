...
def main():
    ...
    for i in range(M):
        if C[i] == mink:
            seg.set(i, 1)
    # print(seg)
    while XI and XI[0][0] < N:
        x, i = XI.popleft()
        ans[i] = A[x]+1
    left = N
    num = seg.prod(0, M)
    for k, L in sorted(K2L.items()):
        # gk回ループ num個
        gk = k - mink
        # print(f"k, L, {gk=}, {num=}")

        while XI and XI[0][0] < left + gk * num:
            x, i = XI.popleft()
            loop, rem = divmod(x-left, num)
            idx = seg.max_right(0, lambda x: x <= rem)
            ans[i] = idx+1
            # print(f"{k=}, L, x, i, {left=}, {loop=}, {rem=}, {idx + 1=}")
            # print(seg)
    ...
    while XI:
        ...
        idx = seg.max_right(0, lambda x: x <= rem)
        ans[i] = idx + 1
        # print(k, L, x, left, loop, rem, idx + 1)
        # print(seg)
