...
class Dijkstra():
    ...
    * ベルマンフォード法より高速なので、負のコストがないならばこちらを使うとよい
    ...
    def shortest_path(self, s):
        ...
        d[s] = 0
        m = [0] * self.V
        heapq.heappush(que, (0, self._A[s], s))  # 始点の(最短距離, 満足度, 頂点番号)をヒープに追加する

        while len(que) != 0:
            cost, con, v = heapq.heappop(que)
            ...
            for i in range(len(self.G[v])):
                ...
                if d[e.to] > d[v] + e.cost or (d[e.to] == d[v] + e.cost and m[e.to] < con + self._A[e.to]):
                    d[e.to] = d[v] + e.cost  # dの更新
                    m[e.to] = con + self._A[e.to]
                    heapq.heappush(que, (d[e.to], m[e.to], e.to))  # キューに新たな最短経路の候補(最短距離, 頂点番号)の情報をpush
        return d, m
