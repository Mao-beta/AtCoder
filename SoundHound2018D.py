...
def main():
    ...
    Y_Dijkstra = Dijkstra(N)
    S_Dijkstra = Dijkstra(N)
    for _ in range(M):
        u, v, a, b = NMI()
        u, v = u-1, v-1
        Y_Dijkstra.add(u, v, a)
        S_Dijkstra.add(u, v, b)
        Y_Dijkstra.add(v, u, a)
        S_Dijkstra.add(v, u, b)

    YS_cost = Y_Dijkstra.shortest_path(S)
    ST_cost = S_Dijkstra.shortest_path(T)

    cost = []
    for k in range(N):
        cost.append(YS_cost[k] + ST_cost[k])
