...
def main():
    N, M, X, Y = NMI()
    trains = [NLI() for _ in range(M)]
    D = Dijkstra(N)
    for a, b, t, k in trains:
        D.add(a-1, b-1, t, k)
        D.add(b-1, a-1, t, k)

    ans = D.shortest_path(X-1)[Y-1]
...
class Dijkstra():
    ...
    * ベルマンフォード法より高速なので、負のコストがないならばこちらを使うとよい
    ...
    def add(self, _from, _to, _cost, _k):
        """ 2頂点と、辺のコストを追加する """
    ...
    def shortest_path(self, s):
        ...
        while len(que) != 0:
            ...
            for i in range(len(self.G[v])):
                ...
                if d[v] % e.k:
                    now_cost = e.cost + (e.k - d[v] % e.k)
                else:
                    now_cost = e.cost

                if d[e.to] > d[v] + now_cost:
                    d[e.to] = d[v] + now_cost  # dの更新
