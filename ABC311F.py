...
def main():
    ...
    dp = segtree([0]*(H+1), lambda x, y: x+y, 0)
    dp.set(H, 1)
    for i in range(W):
        dp2 = segtree([0] * (H + 1), lambda x, y: x + y, 0)
        dp2.set(0, dp.prod(0, H) % MOD99)
        if limH[i] < INF:
            for j in range(1, limH[i]+1):
                dp2.set(j, dp.prod(j-1, H+1) % MOD99)
        else:
