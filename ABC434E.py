...
class mf_graph:
    '''
    https://qiita.com/navel_tos/items/dec64f49151647c00072
...
def main():
    ...
    C = list(C)
    C.sort()
    Z = {c: i for i, c in enumerate(C)}
    ZN = len(Z)

    S = N + ZN
    T = S + 1
    total_nodes = T + 1
    G = mf_graph(total_nodes)
    ...
    for i, (x, r) in enumerate(XR):
        ...
        edges.append((vi, vm, 1))
        edges.append((vi, vp, 1))
        edges.append((S, vi, 1))

    for key, val in Z.items():
        edges.append((val, T, 1))
