...
def mapping(f, a):
    ...
    a_s, a_k = inv(a)
    fb, fc = inv(f)
    fas = (a_s * fb % MOD99 + a_k * fc % MOD99) % MOD99
    fak = a_k % MOD99
    return d(fas, fak)
...
def composition(f, g):
    ...
    fb, fc = inv(f)
    gb, gc = inv(g)
    return d(fb * gb % MOD99, (fb * gc % MOD99 + fc) % MOD99)
...
def main():
    ...
    A = NLI()
    DA = [d(a, 1) for a in A]
    seg = lazy_segtree(DA, op, E, mapping, composition, ID)
    ans = []
    for _ in range(Q):
        q, *X = NMI()
        if q == 0:
