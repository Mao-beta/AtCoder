...
class FPS:
    ...
    @classmethod
    def multipoint_eval(cls, a: list, p: list) -> list:
        m = len(p)
        size = 1 << (m - 1).bit_length()
        tree = [[1] for _ in range(2*size-1)]
        # size-1個、size個(前からm個)
        ...
        # BFSしながら親(上までの剰余)の式をtree[i]で割ったものを新たにtree[i]とする
        tree[0] = cls.mod(a, tree[0])
        for i in range(1, 2*size-1):
            j = (i-1) >> 1
            tree[i] = cls.mod(tree[j], tree[i])
        return tree[size-1:size-1+m]
