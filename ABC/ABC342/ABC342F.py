...
def main():
    ...
    # 最終的にyがiにいる確率(L<=i<=N)、L未満では終わらないので0
    Y = segtree([0]*(N+1), lambda x, y: x+y, 0)
    Y.set(0, 1)
    for i in range(1, N+1):
        l = max(i-D, 0)
        r = min(i, L)
        if l < r:
            Y.set(i, Y.prod(l, r) / D)
    for i in range(L):
        Y.set(i, 0)

    # iから始めたときの勝率　後退解析
    dp = segtree([0]*(N+1), lambda x, y: x+y, 0)
    dp.set(N, 1 - Y.prod(N, N+1))
    for i in range(N-1, -1, -1):
        # 続けるとき、1/D * sum(1個先からD個先までの勝率)
        move = dp.prod(i+1, min(i+D+1, N+1)) / D
        # 留まるとき、Yがi以上N以下でなければ勝ち
        stay = 1 - Y.prod(i, N+1)
        dp.set(i, max(move, stay))
