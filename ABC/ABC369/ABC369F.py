...
def main():
    H, W, N = NMI()
    RC = EI(N)
    RC = [[x-1, y-1, i] for i, (x, y) in enumerate(RC)]
    # 現時点であるcolにおける次手の座標
    C2rc = [[-1, -1, -1] for _ in range(W)]
    seg = segtree([0]*W, max, 0)
    G = [[] for _ in range(N)]
    ...
    for r, c, i in RC:
        ...
        if m == 0:
            ...
            steps[i] = 1
            continue
        tc = seg.max_right(c, lambda x: x < m)
        tr, tc, ti = C2rc[tc]
        G[i].append(ti)
        seg.set(c, m+1)
        C2rc[c] = [r, c, i]
    ...
    while now != [H-1, W-1]:
        ...
        for g in G[now]:
            if steps[g] == s-1:
                nh, nw, ni = RC[g]
                break
        ans.append("D"*abs(nh-now[0]) + "R"*abs(nw-now[1]))
        now = [nh, nw]
    print("".join(ans))
