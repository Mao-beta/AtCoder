...
def main():
    ...
    XY = EI(N)
    INF = 10**15
    VLU = [-x + y for x, y in XY]
    seg_lu = segtree(VLU, max, -INF)
    VRD = [x - y for x, y in XY]
    seg_rd = segtree(VRD, max, -INF)
    VRU = [x + y for x, y in XY]
    seg_ru = segtree(VRU, max, -INF)
    VLD = [-x - y for x, y in XY]
    seg_ld = segtree(VLD, max, -INF)
    ...
    for _ in range(Q):
        ...
        if q == 1:
            ...
            ru = seg_ru.prod(L, R)
            ld = seg_ld.prod(L, R)
            ans = max(ans, lu-(-x+y), rd-(x-y), ru-(x+y), ld-(-x-y))
            print(ans)
